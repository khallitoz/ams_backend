...
const SoftwareAssetDetails: React.FC = () => {
  const router = useRouter();
  const { assetId } = router.query;
  const { getSoftwareAssetDetail, singleStateData } = useAppContext();
  ...
  const getSingleAssetData = async (id: string | string[] | undefined) => {
    ...
    try {
      ...
      const result = await getSoftwareAssetDetail(id);
      ...
      router.replace("/user/softwareassets"); // Redirect to All Assets
    } finally {
      setLoading(false);
    }
  };
    ...
    <Layout>
      <Box>
        ...
        >
          ...
          <Link
            href="/user/softwareassets"
            style={breadcrumbStyles.breadcrumbItem}
          >
...
export default SoftwareAssetDetails;
